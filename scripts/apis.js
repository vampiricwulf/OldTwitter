...
setInterval(() => {
    ...
    for (let i in tweetStorage) {
        if (
            tweetStorage[i].cacheDate &&
            Date.now() - tweetStorage[i].cacheDate > 60000 * 15
        ) {
...
function debugLog(...args) {
    if (typeof vars === "object" && vars.developerMode) {
        if (
            args[0] === "notifications.get" &&
            !document.querySelector(".notifications-modal") &&
...
function updateElementsStats(tweet) {
    ...
    let renderedTweets = Array.from(
        document.querySelectorAll(
            `div.tweet[data-tweet-id="${tr.id_str}"]:not(.tweet-main)`
        )
    );
    for (let t of renderedTweets) {
        if (t.tweet) {
            if (typeof tr.favorite_count === "number")
                t.tweet.favorite_count = tr.favorite_count;
            if (typeof tr.retweet_count === "number")
                t.tweet.retweet_count = tr.retweet_count;
            if (typeof tr.reply_count === "number")
                t.tweet.reply_count = tr.reply_count;
            if (typeof tr.bookmark_count === "number")
        ...
        if (
            ...
            interactViews.innerText = formatLargeNumber(
                tr.ext.views.r.ok.count
            );
        }
    }
}
...
function parseTweet(res) {
    ...
    if (tweet.retweeted_status_result?.result) {
        ...
        if (
            ...
            if (result.legacy.quoted_status) {
                ...
                tweetStorage[result.legacy.quoted_status.id_str].cacheDate =
                    Date.now();
                userStorage[result.legacy.quoted_status.user.id_str] =
                    result.legacy.quoted_status.user;
                userStorage[result.legacy.quoted_status.user.id_str].cacheDate =
                    Date.now();
            } else {
            ...
            result.quoted_status_result.result.tweet.core.user_results.result
                .legacy
        ) {
            result.legacy.quoted_status =
                result.quoted_status_result.result.tweet.legacy;
        ...
        if (tweet.retweeted_status && result.core.user_results.result.legacy) {
            ...
            tweetStorage[tweet.retweeted_status.id_str] =
                tweet.retweeted_status;
            tweetStorage[tweet.retweeted_status.id_str].cacheDate = Date.now();
            userStorage[tweet.retweeted_status.user.id_str] =
                tweet.retweeted_status.user;
            userStorage[tweet.retweeted_status.user.id_str].cacheDate =
                Date.now();
        } else {
    ...
    if (tweet.quoted_status_result && tweet.quoted_status_result.result) {
        ...
        if (tweet.quoted_status) {
            ...
            if (!tweet.quoted_status.user) {
                ...
                userStorage[tweet.quoted_status.user.id_str] =
                    tweet.quoted_status.user;
                userStorage[tweet.quoted_status.user.id_str].cacheDate =
                    Date.now();
            }
        } else {
    ...
    if (
        res.trusted_friends_info_result &&
        res.trusted_friends_info_result.owner_results &&
        res.trusted_friends_info_result.owner_results.result &&
        res.trusted_friends_info_result.owner_results.result.legacy
    ) {
        tweet.trusted_circle_owner =
            res.trusted_friends_info_result.owner_results.result.legacy.screen_name;
    }
    ...
    for (let e of entries) {
        ...
        if (e.entryId.startsWith("tweet-")) {
            ...
            if (e.content.feedbackInfo) {
                tweet.feedback = e.content.feedbackInfo.feedbackKeys
                    .map(
                        (f) =>
                ...
                for (let i = 0; i < items.length; i++) {
                    ...
                    if (item.entryId.includes("-tweet-")) {
                        ...
                        if (
                            res &&
                            res.legacy &&
                            repliesToIgnore.includes(res.legacy.id_str)
                        ) {
                            ignore = true;
                            repliesToIgnore = repliesToIgnore.filter(
                                (r) => r !== res.legacy.id_str
                            );
                            break;
                        }
                    }
                ...
                if (
                    ...
                    if (item.item.itemContent.socialContext) {
                        if (item.item.itemContent.socialContext.topic) {
                            tweet.socialContext =
                                item.item.itemContent.socialContext.topic;
                        } else {
                            tweet.socialContext =
                                item.item.itemContent.socialContext;
                        }
                    }
                    if (i !== items.length - 1) tweet.threadContinuation = true;
                    if (i !== 0) tweet.noTop = true;
                    tweet.hasModeratedReplies =
                        item.item.itemContent.hasModeratedReplies;
                    tweets.push(tweet);
...
const API = {
    account: {
        verifyCredentials: () => {
            return new Promise((resolve, reject) => {
                chrome.storage.local.get(["credentials"], (d) => {
                    if (
                        d.credentials &&
                        Date.now() - d.credentials.date < 15000
                    ) {
            ...
            return new Promise((resolve, reject) => {
                chrome.storage.local.get(["accountsList"], (d) => {
                    if (
                        cache &&
                        d.accountsList &&
                        Date.now() - d.accountsList.date < 60000 * 5
                    ) {
        ...
        updateProfile: (data) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `https://api.${location.hostname}/1.1/account/update_profile.json`,
                    {
                        headers: {
                            authorization: OLDTWITTER_CONFIG.public_token,
                            "x-csrf-token": OLDTWITTER_CONFIG.csrf,
        ...
        getSettings: () => {
            return new Promise((resolve, reject) => {
                chrome.storage.local.get(["twitterSettings"], (d) => {
                    if (
                        d.twitterSettings &&
                        Date.now() - d.twitterSettings.date < 60000 * 10
                    ) {
        ...
        getChronological: (max_id) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `https://api.${
                        location.hostname
                    }/1.1/statuses/home_timeline.json?count=40&include_my_retweet=1&cards_platform=Web-12&include_cards=1&include_entities=1&include_user_entities=1&include_cards=1&send_error_codes=1&tweet_mode=extended&include_ext_alt_text=true&ext=views%2CmediaStats%2CverifiedType%2CisBlueVerified&include_reply_count=true${
        ...
        getChronologicalV2: (cursor, count = 40, useDiffKey) => {
            return new Promise((resolve, reject) => {
                ...
                fetch(
                    ...
                    .then((response) => response.json())
                    .then(async (data) => {
                        debugLog("timeline.getChronologicalV2", "start", {
                            cursor,
                            count,
                            data,
                        });
            ...
            return new Promise((resolve, reject) => {
                fetch(
                    `/i/api/2/timeline/home.json?${
                        cursor ? `cursor=${cursor.replace(/\+/g, "%2B")}&` : ""
                    ...
                    .then((response) => response.json())
                    .then((data) => {
                        debugLog("timeline.getAlgorithmical", "start", {
                            cursor,
                            count,
                            data,
                        });
                        ...
                        let entries = data.timeline.instructions.find(
                            (i) => i.addEntries
                        );
                        if (!entries) {
                            return reject({
                        ...
                        for (let i = 0; i < entries.length; i++) {
                            ...
                            if (
                                e.content.tweet.promotedMetadata &&
                                !vars.enableAd
                            )
                                continue;
                            ...
                            if (e.feedbackInfo) {
                                tweet.feedback =
                                    e.feedbackInfo.feedbackKeys.map(
                                        (f) =>
                                            data.timeline.responseObjects
                                                .feedbackActions[f]
                                    );
                                if (tweet.feedback)
                                    tweet.feedbackMetadata =
                                        e.feedbackInfo.feedbackMetadata;
                            }
                            if (tweet.retweeted_status_id_str) {
                                tweet.retweeted_status =
                                    tweets[tweet.retweeted_status_id_str];
                                if (tweet.retweeted_status) {
                                    tweet.retweeted_status.user =
                                        users[
                            ...
                            if (tweet.quoted_status_id_str) {
                                tweet.quoted_status =
                                    tweets[tweet.quoted_status_id_str];
                                if (tweet.quoted_status) {
                                    tweet.quoted_status.user =
                                        users[tweet.quoted_status.user_id_str];
                            ...
                            if (e.content.tweet.socialContext) {
                                if (
                                    e.content.tweet.socialContext.topicContext
                                ) {
                                    tweet.socialContext =
                                        data.globalObjects.topics[
                                            e.content.tweet.socialContext.topicContext.topicId
                                        ];
                                } else {
                                    tweet.socialContext =
                                        e.content.tweet.socialContext.generalContext;
                                }
                            }
                            list.push(tweet);
                        }
                        ...
                        let out = {
                            list,
                            cursorBottom: cb
                                ? cb.content.operation.cursor.value
                                : undefined,
                            cursorTop: ct
                                ? ct.content.operation.cursor.value
                                : undefined,
                            suspended: entries.find(
                                (e) =>
                                    e.entryId ===
                        ...
                        debugLog("timeline.getAlgorithmical", "end", {
                            cursor,
                            count,
                            out,
                        });
        ...
        getAlgorithmicalV2: (
            ...
            return new Promise((resolve, reject) => {
                ...
                fetch(`/i/api/graphql/k3YiLNE_MAy5J-NANLERdg/HomeTimeline`, {
                    ...
                    .then((response) => response.json())
                    .then((data) => {
                        debugLog("timeline.getAlgorithmicalV2", "start", {
                            cursor,
                            count,
                            data,
                        });
        ...
        getAlgorithmicalV2WithCache: (seenTweetIds = []) => {
            return new Promise((resolve, reject) => {
                chrome.storage.local.get(["algoTimeline"], (d) => {
                    if (
                        d.algoTimeline &&
                        Date.now() - d.algoTimeline.date < 60000 * 3
                    ) {
                        if (d.algoTimeline.data && d.algoTimeline.data.list) {
                            d.algoTimeline.data.list =
                                d.algoTimeline.data.list.filter(
                                    (t) => !seenTweetIds.includes(t.id_str)
                                );
                            if (d.algoTimeline.data.list.length > 5) {
                                debugLog(
                                    "timeline.getAlgorithmicalV2WithCache",
                                    "cache",
                                    d.algoTimeline.data,
                                    seenTweetIds
                                );
                    ...
                    API.timeline
                        .getAlgorithmicalV2(undefined, 40, seenTweetIds)
                        .then((data) => {
                            chrome.storage.local.set({
                                algoTimeline: {
                                    date: Date.now(),
    ...
    discover: {
            ...
            return new Promise((resolve, reject) => {
                chrome.storage.local.get(["discoverData"], (d) => {
                    if (
                        cache &&
                        d.discoverData &&
                        Date.now() - d.discoverData.date < 60000 * 10
                    ) {
                        debugLog(
                            "discover.getPeople",
                            "cache",
        ...
        getSimilarPeople: (id, cache = true, by_screen_name = false) => {
            return new Promise((resolve, reject) => {
                chrome.storage.local.get([`peopleRecommendations`], (d) => {
                    ...
                    fetch(
                        `/i/api/1.1/users/recommendations.json?include_profile_interstitial_type=1&include_blocking=1&include_blocked_by=1&include_followed_by=1&include_want_retweets=1&include_mute_edge=1&include_can_dm=1&include_can_media_tag=1&include_ext_has_nft_avatar=1&skip_status=1&&pc=true&display_location=profile_accounts_sidebar&limit=4&${
                            by_screen_name ? "screen_name" : "user_id"
                        }=${id}&ext=mediaStats%2CverifiedType%2CisBlueVerified%2ChighlightedLabel%2ChasNftAvatar%2CvoiceInfo%2Cenrichments%2CsuperFollowMetadata%2CunmentionInfo%2Ccollab_control`,
                        {
                            headers: {
                                authorization: OLDTWITTER_CONFIG.public_token,
                                "x-csrf-token": OLDTWITTER_CONFIG.csrf,
                        ...
                        .then((i) => i.json())
                        .then((data) => {
                            debugLog("discover.getSimilarPeople", {
                                id,
                                cache,
                                by_screen_name,
                                data,
        ...
        getTrends: () => {
            return new Promise((resolve, reject) => {
                chrome.storage.local.get(["trends"], (d) => {
                    ...
                    fetch(
                        `https://api.${location.hostname}/1.1/trends/plus.json?max_trends=8`,
                        {
                            headers: {
                                authorization: OLDTWITTER_CONFIG.public_token,
                                "x-csrf-token": OLDTWITTER_CONFIG.csrf,
        ...
        getTrendsV2: (cache) => {
            return new Promise((resolve, reject) => {
                chrome.storage.local.get(["trendsv2"], (d) => {
                    if (
                        d.trendsv2 &&
                        Date.now() - d.trendsv2.date < 60000 * 5 &&
                        cache
                    ) {
                        debugLog(
                            "discover.getTrendsV2",
                            "cache",
                            d.trendsv2.data
                        );
                    ...
                    fetch(
                        ...
                        .then((i) => i.json())
                        .then((d) => {
                            debugLog("discover.getTrendsV2", "start", {
                                cache,
                                data: d,
                            });
                            ...
                            trends.forEach((trend) => {
                                if (
                                    ...
                                    return;
                                let desc =
                                    trend.item.content.trend.trendMetadata
                                        .domainContext;
                                if (String(desc).includes("undefined")) {
                                    //maybe promoted trends?
                                    desc = ``;
                                    if (
                                        trend.item.content.trend.trendMetadata
                                            .metaDescription
                                ...
                                if (
                                    (desc.startsWith("Promoted by") /*en*/ ||
                                        desc.startsWith(
                                            "Promocionado por"
                                        ) /*es*/ ||
                                        desc.startsWith(
                                ...
                                if (desc.endsWith(" posts")) {
                                    //why they changed to lower-case
                                    desc = desc.replace(
                                        ` posts`,
                                        ` ${LOC.tweets.message}`
                                    );
                            ...
                            debugLog("discover.getTrendsV2", "end", {
                                cache,
                                data: { modules: data },
                            });
        ...
        getHashflags: () => {
            return new Promise((resolve, reject) => {
                chrome.storage.local.get(["hashflags"], (d) => {
                    if (
                        d.hashflags &&
                        Date.now() - d.hashflags.date < 60000 * 60 * 4
                    ) {
        ...
        getHashflagsV2: () => {
            ...
            return new Promise((resolve, reject) => {
                ...
                if (
                    hashflagStorage &&
                    Date.now() - hashflagStorage.date < 60000 * 60 * 4
                ) {
    ...
    notifications: {
            ...
            return new Promise((resolve, reject) => {
                chrome.storage.local.get(["unreadCount"], (d) => {
                    if (
                        cache &&
                        d.unreadCount &&
                        Date.now() - d.unreadCount.date < 30000 &&
                        d.unreadCount.userId == userId
                    ) {
                        return resolve(d.unreadCount.data);
                    }
                    if (userId == user.id_str) userId = "";
                    let multiAuthHeader = userId
                        ? { "x-web-auth-multi-user-id": userId }
                        : {};
                    fetch(
                        `/i/api/2/badge_count/badge_count.json?supports_ntab_urt=1`,
                        {
                            headers: {
                                authorization: OLDTWITTER_CONFIG.public_token,
                                "x-csrf-token": OLDTWITTER_CONFIG.csrf,
            ...
            return new Promise((resolve, reject) => {
                chrome.storage.local.get(["notifications"], (d) => {
                    if (cache) {
                        if (
                            d.notifications &&
                            Date.now() - d.notifications.date < 60000 &&
                            !cursor &&
                            !onlyMentions
                        ) {
                            debugLog(
                        ...
                        if (
                            d.notifications &&
                            Date.now() - d.notifications.date < 15000 &&
                            !cursor &&
                            !onlyMentions
                        ) {
                            debugLog(
                    ...
                    if (!cursor && !onlyMentions) {
                        if (loadingNotifs) {
                            return loadingNotifs.listeners.push([
                                resolve,
                                reject,
                            ]);
                        } else {
                    ...
                    fetch(
                        ...
                        .then((i) => i.json())
                        .then((data) => {
                            debugLog("notifications.get", "start", {
                                cursor,
                                onlyMentions,
                                data,
                            });
                            if (data.errors && data.errors[0].code === 32) {
                                if (!cursor && !onlyMentions) {
                                    loadingNotifs.listeners.forEach((l) =>
                                        l[1]("Not logged in")
                                    );
                            ...
                            let cursorTop = entries.find((e) =>
                                e.entryId.startsWith("cursor-top-")
                            ).content.operation.cursor.value;
                            let cursorBottom = entries.find((e) =>
                                e.entryId.startsWith("cursor-bottom-")
                            )?.content.operation.cursor.value;
                            let unreadBefore = +data.timeline.instructions.find(
                                (i) => i.markEntriesUnreadGreaterThanSortIndex
                            ).markEntriesUnreadGreaterThanSortIndex.sortIndex;
                            let unreadNotifications = 0;
                            let res = [];
                            for (let i in entries) {
                                if (
                                    !entries[i].entryId.startsWith(
                                        "notification-"
                                    )
                                )
                                    continue;
                                ...
                                if (e.content.notification) {
                                    let n =
                                        data.globalObjects.notifications[
                                            e.content.notification.id
                                        ];
                                    if (!n) continue;
                                    if (e.feedbackInfo) {
                                        n.feedback =
                                            data.timeline.responseObjects.feedbackActions[
                                                e.feedbackInfo.feedbackKeys[0]
                                            ];
                                        if (n.feedback)
                                            n.feedback.metadata =
                                                e.feedbackInfo.feedbackMetadata;
                                    }
                                    let tweet = n.template
                                        .aggregateUserActionsV1.targetObjects[0]
                                        ? data.globalObjects.tweets[
                                              n.template.aggregateUserActionsV1
                                                  .targetObjects[0].tweet.id
                                          ]
                                        : { full_text: "" };
                                    ...
                                    t.user =
                                        data.globalObjects.users[t.user_id_str];
                                ...
                                chrome.storage.local.get(
                                    ...
                                    (d) => {
                                        if (
                                            d.notifications &&
                                            d.notifications.data &&
                                            d.notifications.data.list
                                        ) {
                                            d.notifications.data.list =
                                                d.notifications.data.list.filter(
                                                    (n) =>
                                                        !res.find(
                                                            (r) =>
                                                                r.type ===
                                            ...
                                            d.notifications.data.cursorTop =
                                                cursorTop;
                                            chrome.storage.local.set(
                                                {
                                                    notifications:
        ...
        getDeviceFollowTweets: (cursor) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `/i/api/2/notifications/device_follow.json?include_profile_interstitial_type=1&include_blocking=1&include_blocked_by=1&include_followed_by=1&include_want_retweets=1&include_mute_edge=1&include_can_dm=1&include_can_media_tag=1&include_ext_has_nft_avatar=1&skip_status=1&cards_platform=Web-12&include_cards=1&include_ext_alt_text=true&include_ext_limited_action_results=false&include_quote_count=true&include_reply_count=1&tweet_mode=extended&include_ext_collab_control=true&include_entities=true&include_user_entities=true&include_ext_media_color=true&include_ext_media_availability=true&include_ext_sensitive_media_warning=true&include_ext_trusted_friends_metadata=true&send_error_codes=true&simple_quoted_tweet=true&count=20&ext=mediaStats%2CverifiedType%2CisBlueVerified%2ChighlightedLabel%2ChasNftAvatar%2CvoiceInfo%2Cenrichments%2CsuperFollowMetadata%2CunmentionInfo%2CeditControl%2Ccollab_control%2Cvibe${
                        cursor ? `&cursor=${cursor}` : ""
                    }`,
                    {
                        headers: {
                    ...
                    .then((data) => {
                        debugLog(
                            "notifications.getDeviceFollowTweets",
                            "start",
                            {
                                cursor,
                                data,
                        ...
                        let tweets = entries
                            .filter((i) => i.entryId.startsWith("tweet-"))
                            .map(
                                (i) =>
                                    data.globalObjects.tweets[
                                        i.content.item.content.tweet.id
                                    ]
                            );
                        for (let i in tweets) {
                            tweets[i].user =
                                data.globalObjects.users[tweets[i].user_id_str];
                            if (tweets[i].quoted_status_id_str) {
                                tweets[i].quoted_status =
                                    data.globalObjects.tweets[
                                        tweets[i].quoted_status_id_str
                                    ];
                                if (tweets[i].quoted_status)
                                    tweets[i].quoted_status.user =
                                        data.globalObjects.users[
                                            tweets[i].quoted_status.user_id_str
                                        ];
                            }
                            if (tweets[i].retweeted_status_id_str) {
                                tweets[i].retweeted_status =
                                    data.globalObjects.tweets[
                                        tweets[i].retweeted_status_id_str
                                    ];
                                if (tweets[i].retweeted_status)
                                    tweets[i].retweeted_status.user =
                                        data.globalObjects.users[
                                            tweets[
                                                i
                                            ].retweeted_status.user_id_str
                                        ];
                            }
                        }
                        let newCursor = entries.find((e) =>
                            e.entryId.startsWith("cursor-bottom-")
                        );
                        if (newCursor) {
                            newCursor =
                                newCursor.content.operation.cursor.value;
                        }
                        let out = {
                            list: tweets,
                            cursor: newCursor,
                    ...
                    .then((data) => {
                        ...
                        for (let i in entries) {
                            ...
                            if (e.entryId.startsWith("tweet-")) {
                                let tweet =
                                    data.globalObjects.tweets[
                                        e.content.item.content.tweet.id
                                    ];
                                tweet.user =
                                    data.globalObjects.users[tweet.user_id_str];
                                if (tweet.quoted_status_id_str) {
                                    tweet.quoted_status =
                                        data.globalObjects.tweets[
                                            tweet.quoted_status_id_str
                                ...
                                let id =
                                    e.content.timelineModule.items[0].item
                                        .content.tweet.id;
    ...
    user: {
        get: (val, byId = true) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `https://api.${location.hostname}/1.1/users/show.json?${
                ...
                fetch(
                    ...
                    .then((data) => {
                        ...
                        if (result.legacy_extended_profile.birthdate) {
                            result.legacy.birthdate =
                                result.legacy_extended_profile.birthdate;
                        }
                        if (result.professional) {
                            result.legacy.professional = result.professional;
                        }
                        if (
                            result.affiliates_highlighted_label &&
                            result.affiliates_highlighted_label.label
                        ) {
                            result.legacy.affiliates_highlighted_label =
                                result.affiliates_highlighted_label.label;
                        }
                        if (
        ...
        follow: (screen_name) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `https://api.${location.hostname}/1.1/friendships/create.json`,
                    {
                        method: "POST",
                        headers: {
                            authorization: OLDTWITTER_CONFIG.public_token,
                    ...
                    .then((data) => {
                        ...
                        chrome.storage.local.get(
                            ...
                            async (d) => {
                                ...
                                if (!sortedFollowers[user.id_str].followers)
                                    return;
                                if (
                                    sortedFollowers[user.id_str].followers
                                        .length === 0
                                )
                                    return;

                                let index = sortedFollowers[
                                    user.id_str
                                ].followers.findIndex(
                                    (f) => f[2] === screen_name
                                );
                        ...
                        let cachedUser = Object.values(userStorage).find(
                            (u) =>
                                u.screen_name.toLowerCase() ===
        ...
        unfollow: (screen_name) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `https://api.${location.hostname}/1.1/friendships/destroy.json`,
                    {
                        method: "POST",
                        headers: {
                            authorization: OLDTWITTER_CONFIG.public_token,
        ...
        getTweets: (id, max_id, replies = false) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `https://api.${
                        location.hostname
                    }/1.1/statuses/user_timeline.json?count=100&exclude_replies=${!replies}&include_my_retweet=1&include_rts=1&user_id=${id}${
                        max_id ? `&max_id=${max_id}` : ""
                    }&cards_platform=Web-12&include_entities=1&include_user_entities=1&include_cards=1&send_error_codes=1&tweet_mode=extended&include_ext_alt_text=true&include_reply_count=true&ext=views%2CmediaStats%2CverifiedType%2CisBlueVerified`,
                    {
                        headers: {
                            authorization: OLDTWITTER_CONFIG.public_token,
                            "x-csrf-token": OLDTWITTER_CONFIG.csrf,
        ...
        getTweetsV2: (id, cursor, replies = false, useDiffKey) => {
            return new Promise((resolve, reject) => {
                ...
                if (replies) {
                    ...
                    api = "pz0IHaV_t7T4HJavqqqcIA/UserTweetsAndReplies";
                    fieldToggles = { withArticlePlainText: false };
                }

                fetch(
                    `/i/api/graphql/${api}?variables=${encodeURIComponent(
                        ...
                        .then((data) => {
                            ...
                            if (!ix)
                                ix =
                                    data.data?.user?.result?.timeline?.timeline
                                        ?.instructions;
                            if (data.errors && data.errors[0] && !ix) {
                                if (
                                    ...
                                    localStorage.hitRateLimit =
                                        Date.now() + 1000 * 60 * 10;
                                    return API.user
                                        .getTweetsV2(id, cursor, replies, true)
                                        .then(resolve)
                                        .catch(reject);
                                }
                                return reject(data.errors[0].message);
                            ...
                            for (let entry of entries) {
                                if (entry.entryId.startsWith("tweet-")) {
                                    ...
                                    if (tweet) {
                                        tweet.hasModeratedReplies =
                                            entry.content.itemContent.hasModeratedReplies;
                                        if (replies) {
                                            tweet.nonReply = true;
                                        }
                                        tweets.push(tweet);
                            ...
                            if (
                                ...
                                if (pinnedTweet) {
                                    pinnedTweet.hasModeratedReplies =
                                        pinEntry.entry.content.itemContent.hasModeratedReplies;
                                }
                            } else if (pinEntry) {
                                console.error(
                                    "Weird bug, pinEntry is there but no entry or content",
                                    pinEntry
                                );
                            }
        ...
        getMediaTweets: (id, cursor) => {
            return new Promise((resolve, reject) => {
                ...
                fetch(
                    `/i/api/graphql/1dmA2m-qIsGm2XfqQtcD3A/UserMedia?variables=${encodeURIComponent(
                        JSON.stringify(variables)
                    )}&features=${encodeURIComponent(
                        JSON.stringify(features)
                    )}&fieldToggles=${encodeURIComponent(
                    ...
                    .then((i) => i.json())
                    .then((data) => {
                        debugLog("user.getMediaTweets", "start", {
                            id,
                            cursor,
                            data,
                        });
                        ...
                        if (isGrid) {
                            ...
                            tweets = items
                                .filter((e) =>
                                    e.entryId.startsWith(
                                        "profile-grid-0-tweet-"
                                    )
                                )
                                .map((t) => {
                                    let tweet = parseTweet(
                                        t.item.itemContent.tweet_results.result
                                    );
                                    if (tweet) {
                                        tweet.hasModeratedReplies =
                                            t.item.itemContent.hasModeratedReplies;
                                    }
                                    return tweet;
                                })
                                .filter((i) => i);
                            ...
                            tweets = entries
                                ...
                                .map((t) => {
                                    let tweet = parseTweet(
                                        t.content.itemContent.tweet_results
                                            .result
                                    );
                                    if (tweet) {
                                        tweet.hasModeratedReplies =
                                            t.content.itemContent.hasModeratedReplies;
                                    }
                                    return tweet;
                                })
                                .filter((i) => i);
        ...
        friendsFollowing: (val, by_id = true) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `/i/api/1.1/friends/following/list.json?include_profile_interstitial_type=1&include_blocking=1&include_blocked_by=1&include_followed_by=1&include_want_retweets=1&include_mute_edge=1&include_can_dm=1&include_can_media_tag=1&include_ext_has_nft_avatar=1&skip_status=1&cursor=-1&${
                        by_id ? `user_id=${val}` : `screen_name=${val}`
                    }&count=10&with_total_count=true`,
                    {
                        headers: {
                            authorization: OLDTWITTER_CONFIG.public_token,
                            "x-csrf-token": OLDTWITTER_CONFIG.csrf,
        ...
        getRelationship: (id) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `https://api.${location.hostname}/1.1/friendships/show.json?source_id=${id}&target_screen_name=JinjersTemple&cards_platform=Web-13&include_entities=1&include_user_entities=1&include_cards=1&send_error_codes=1&tweet_mode=extended&include_ext_alt_text=true&include_reply_count=true`,
                    {
                        headers: {
                            authorization: OLDTWITTER_CONFIG.public_token,
                            "x-csrf-token": OLDTWITTER_CONFIG.csrf,
        ...
        getFavorites: (id, cursor) => {
            return new Promise((resolve, reject) => {
                ...
                fetch(
                    `/i/api/graphql/vni8vUvtZvJoIsl49VPudg/Likes?variables=${encodeURIComponent(
                        JSON.stringify(obj)
                    )}&features=${encodeURIComponent(
                        JSON.stringify({
                    ...
                    .then((i) => i.json())
                    .then((data) => {
                        debugLog("user.getFavorites", "start", {
                            id,
                            cursor,
                            data,
                        });
                        ...
                        let out = {
                            tl: data.data.user.result.timeline_v2.timeline.instructions[0].entries
                                ...
                                .filter((e) => e),
                            cursor: data.data.user.result.timeline_v2.timeline.instructions[0].entries.find(
                                (e) => e.entryId.startsWith("cursor-bottom")
                            ).content.value,
                        };
        ...
        getFollowing: (id, cursor) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `https://${
                        location.hostname
                    }/i/api/1.1/friends/list.json?include_followed_by=1&user_id=${id}&count=100${
                        cursor ? `&cursor=${cursor}` : ""
                    }`,
                    {
                        headers: {
        ...
        getFollowers: (id, cursor, count = 100) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `https://${
                        location.hostname
                    }/i/api/1.1/followers/list.json?include_followed_by=1&user_id=${id}&count=${count}${
                        cursor ? `&cursor=${cursor}` : ""
                    }`,
                    {
                        headers: {
        ...
        getFollowingV2: (id, cursor) => {
            return new Promise((resolve, reject) => {
                ...
                if (cursor) obj.cursor = cursor;
                fetch(
                    `/i/api/graphql/t-BPOrMIduGUJWO_LxcvNQ/Following?variables=${encodeURIComponent(
                        JSON.stringify(obj)
                    )}&features=${encodeURIComponent(
                        JSON.stringify({
                    ...
                    .then((i) => i.json())
                    .then((data) => {
                        debugLog("user.getFollowingV2", "start", {
                            id,
                            cursor,
                            data,
                        });
        ...
        getFollowersV2: (id, cursor, count = 100) => {
            return new Promise((resolve, reject) => {
                ...
                if (cursor) obj.cursor = cursor;
                fetch(
                    `/i/api/graphql/fJSopkDA3UP9priyce4RgQ/Followers?variables=${encodeURIComponent(
                        JSON.stringify(obj)
                    )}&features=${encodeURIComponent(
                        JSON.stringify({
                            dont_mention_me_view_api_enabled: true,
                    ...
                    .then((i) => i.json())
                    .then((data) => {
                        debugLog("user.getFollowersV2", "start", {
                            id,
                            cursor,
                            data,
                        });
        ...
        getFollowersYouFollow: (id, cursor) => {
            return new Promise((resolve, reject) => {
                ...
                if (cursor) obj.cursor = cursor;
                fetch(
                    `/i/api/graphql/m8AXvuS9H0aAI09J3ISOrw/FollowersYouKnow?variables=${encodeURIComponent(
                        JSON.stringify(obj)
                    )}&features=${encodeURIComponent(
                        JSON.stringify({
                            rweb_lists_timeline_redesign_enabled: false,
                    ...
                    .then((i) => i.json())
                    .then((data) => {
                        debugLog("user.getFollowersYouFollow", "start", {
                            id,
                            cursor,
                            data,
                        });
        ...
        translateBio: (id) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `/i/api/1.1/strato/column/None/profileUserId=${id},destinationLanguage=None,translationSource=Some(Google)/translation/service/translateProfile`,
                    {
                        headers: {
                            authorization: OLDTWITTER_CONFIG.public_token,
                            "x-csrf-token": OLDTWITTER_CONFIG.csrf,
        ...
        getLists: (id) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `/i/api/graphql/mLKOzzVOWUycBiExBT1gjg/CombinedLists?variables=${encodeURIComponent(
                        JSON.stringify({
                            userId: id,
                            count: 100,
                            withSuperFollowsUserFields: true,
    ...
    tweet: {
        post: (data) => {
            ...
            return new Promise((resolve, reject) => {
                fetch(
                    `https://api.${location.hostname}/1.1/statuses/update.json`,
                    {
                        method: "POST",
                        headers: {
                            authorization: OLDTWITTER_CONFIG.public_token,
        ...
        */
        postV2: (tweet) => {
            return new Promise((resolve, reject) => {
                ...
                if (tweet.media) {
                    variables.media.media_entities = tweet.media.map((i) => ({
                        media_id: i,
                        tagged_users: [],
                    }));
                    if (tweet.sensitive) {
                ...
                if (tweet.circle) {
                    variables.trusted_friends_control_options = {
                        trusted_friends_list_id: tweet.circle,
                    };
                }
                ...
                if (tweet.in_reply_to_tweet_id) {
                    ...
                    if (tweet.exclude_reply_user_ids) {
                        if (typeof tweet.exclude_reply_user_ids === "string") {
                            tweet.exclude_reply_user_ids =
                                tweet.exclude_reply_user_ids.split(",");
                        }
                ...
                fetch(
                    ...
                    .then((data) => {
                        ...
                        let ct = data.data.create_tweet
                            ? data.data.create_tweet
                            : data.data.notetweet_create;
                        let result = ct.tweet_results.result;
                        let tweet = parseTweet(result);
                        if (
                            result.trusted_friends_info_result &&
                            !tweet.limited_actions
                        ) {
                            tweet.limited_actions =
                                "limit_trusted_friends_tweet";
                        }
        ...
        postScheduled: (data) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `/i/api/graphql/LCVzRQGxOaGnOnYH01NQXg/CreateScheduledTweet`,
                    {
                        method: "POST",
                        headers: {
                            authorization: OLDTWITTER_CONFIG.public_token,
        ...
        getV2: (id, useDiffKey) => {
            return new Promise((resolve, reject) => {
                chrome.storage.local.get(["tweetDetails"], (d) => {
                    ...
                    if (
                        d.tweetDetails[id] &&
                        Date.now() - d.tweetDetails[id].date < 60000 * 3
                    ) {
                        debugLog(
                            "tweet.getV2",
                            "cache",
                            id,
                            d.tweetDetails[id].data
                        );
                        return resolve(d.tweetDetails[id].data);
                    }
                    if (loadingDetails[id]) {
                        if (!useDiffKey)
                            return loadingDetails[id].listeners.push([
                                resolve,
                                reject,
                            ]);
                    } else {
        ...
        vote: (api, tweet_id, card_uri, card_name, selected_choice) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `https://caps.${location.hostname}/v2/capi/${
                        api.split("//")[1]
                    }`,
                    {
                        headers: {
                        ...
                        credentials: "include",
                        method: "post",
                        body: `twitter%3Astring%3Acard_uri=${encodeURIComponent(
                            card_uri
                        )}&twitter%3Along%3Aoriginal_tweet_id=${tweet_id}&twitter%3Astring%3Aresponse_card_name=${card_name}&twitter%3Astring%3Acards_platform=Web-12&twitter%3Astring%3Aselected_choice=${selected_choice}`,
                    }
                )
                    .then((response) => response.json())
                    .then((data) => {
        ...
        createCard: (card_data) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `https://caps.${location.hostname}/v2/cards/create.json`,
                    {
                        headers: {
                            authorization: OLDTWITTER_CONFIG.public_token,
                            "x-csrf-token": OLDTWITTER_CONFIG.csrf,
            ...
            return new Promise((resolve, reject) => {
                ...
                chrome.storage.local.get(["tweetReplies"], async (d) => {
                    ...
                    if (!cursor) {
                        if (
                            d.tweetReplies[id] &&
                            Date.now() - d.tweetReplies[id].date < 60000
                        ) {
                            return resolve(d.tweetReplies[id].data);
                        }
                        if (loadingReplies[id]) {
                            return loadingReplies[id].listeners.push([
                                resolve,
                                reject,
                            ]);
                        } else {
                    ...
                    fetch(
                        `https://api.${
                            location.hostname
                        }/2/timeline/conversation/${id}.json?${
                            cursor ? `cursor=${cursor}` : ""
                        }&count=30&include_reply_count=true&cards_platform=Web-13&include_entities=1&include_user_entities=1&include_cards=1&send_error_codes=1&tweet_mode=extended&include_ext_alt_text=true&ext=views%2CmediaStats%2CverifiedType%2CisBlueVerified%2CnoteTweet`,
                        {
                            headers: {
                                authorization: OLDTWITTER_CONFIG.public_token,
                                "x-csrf-token": OLDTWITTER_CONFIG.csrf,
                        ...
                        .then((data) => {
                            ...
                            let ae = data.timeline.instructions.find(
                                (i) => i.addEntries
                            );
                            if (!ae) {
                                let out = {
                            ...
                            for (let i = 0; i < entries.length; i++) {
                                ...
                                if (e.entryId.startsWith("tweet-")) {
                                    ...
                                    if (
                                        e.content.item.content.tombstone.tweet
                                    ) {
                                        let tweet =
                                            tweetData[
                                                e.content.item.content.tombstone
                                                    .tweet.id
                                            ];
                                        let user = userData[tweet.user_id_str];
                                        tweet.id_str =
                                            e.content.item.content.tombstone.tweet.id;
                                        tweet.user = user;
                                        if (tweet.quoted_status_id_str) {
                                            tweet.quoted_status =
                                                tweetData[
                                        ...
                                        tweet.tombstone =
                                            e.content.item.content.tombstone.tombstoneInfo.text;
                                        list.push({
                                            type:
                                                tweet.id_str === id
                                                    ? "mainTweet"
                                        ...
                                        list.push({
                                            type: "tombstone",
                                            data: e.content.item.content
                                                .tombstone.tombstoneInfo.text,
                                        });
                                    }
                                } else if (
                                    ...
                                    let thread =
                                        e.content.item.content.conversationThread.conversationComponents.filter(
                                            (c) => c.conversationTweetComponent
                                        );
                                    let threadList = [];
                                    for (let j = 0; j < thread.length; j++) {
                                        ...
                                        let tweet =
                                            tweetData[
                                                t.conversationTweetComponent
                                                    .tweet.id
                                            ];
                                        if (!tweet) continue;
                                        let user = userData[tweet.user_id_str];
                                        tweet.id_str =
                                            t.conversationTweetComponent.tweet.id;
                                        if (tweet.quoted_status_id_str) {
                                            tweet.quoted_status =
                                                tweetData[
                                                    tweet.quoted_status_id_str
            ...
            return new Promise((resolve, reject) => {
                chrome.storage.local.get(["tweetReplies"], (d) => {
                    ...
                    fetch(
                        ...
                        .then((i) => i.json())
                        .then((data) => {
                            debugLog("tweet.getRepliesV2", "start", {
                                cursor,
                                data,
                            });
                            if (data.errors && data.errors[0] && !data.data) {
                            ...
                            for (let i = 0; i < entries.length; i++) {
                                ...
                                if (e.entryId.startsWith("tweet-")) {
                                    ...
                                    if (tweetData.tombstone) {
                                        let text =
                                            tweetData.tombstone.text.text;
                                        if (
                                            tweetData.tombstone.text.entities &&
                                            tweetData.tombstone.text.entities
                                                .length > 0
                                        ) {
                                            let en =
                                                tweetData.tombstone.text
                                                    .entities[0];
                                            text =
                                                text.slice(0, en.fromIndex) +
                                                `<a href="${en.ref.url}" target="_blank">` +
                                                text.slice(
                                                    en.fromIndex,
                                                    en.toIndex
                                                ) +
                                                "</a>" +
                                                text.slice(en.toIndex);
                                        }
                                        let tombstoneTweetId =
                                            e.entryId.slice(6);
                                        let replyTweet = entries.find(
                                            (i) =>
                                                i &&
                                                i.content &&
                                                i.content.itemContent &&
                                                i.content.itemContent
                                                    .tweet_results &&
                                                i.content.itemContent
                                                    .tweet_results.result &&
                                    ...
                                    if (
                                        ...
                                        e.content.itemContent &&
                                        e.content.itemContent.tombstoneInfo
                                    ) {
                                        let richText =
                                            e.content.itemContent.tombstoneInfo
                                                .richText;
            ...
            return new Promise((resolve, reject) => {
                ...
                chrome.storage.local.get(["tweetLikers"], (d) => {
                    ...
                    if (!cursor) {
                        if (
                            d.tweetLikers[id] &&
                            Date.now() - d.tweetLikers[id].date < 60000
                        ) {
                            debugLog(
                                "tweet.getLikers",
                                "cache",
                                d.tweetLikers[id].data
                            );
                            return resolve(d.tweetLikers[id].data);
                        }
                        if (loadingLikers[id]) {
                            return loadingLikers[id].listeners.push([
                                resolve,
                                reject,
                            ]);
                        } else {
                    ...
                    fetch(
                        `/i/api/graphql/RMoTahkos95Jcdw-UWlZog/Favoriters?variables=${encodeURIComponent(
                            JSON.stringify(obj)
                        )}&features=${encodeURIComponent(
                            JSON.stringify({
                                dont_mention_me_view_api_enabled: true,
                        ...
                        .then((i) => i.json())
                        .then((data) => {
                            debugLog("tweet.getLikers", "start", {
                                id,
                                cursor,
                                data,
                            });
        ...
        getRetweeters: (id, cursor) => {
            return new Promise((resolve, reject) => {
                ...
                if (cursor) obj.cursor = cursor;
                fetch(
                    `/i/api/graphql/qVWT1Tn1FiklyVDqYiOhLg/Retweeters?variables=${encodeURIComponent(
                        JSON.stringify(obj)
                    )}&features=${encodeURIComponent(
                        JSON.stringify({
                            dont_mention_me_view_api_enabled: true,
                    ...
                    .then((i) => i.json())
                    .then((data) => {
                        debugLog("tweet.getRetweeters", "start", {
                            id,
                            cursor,
                            data,
                        });
                        ...
                        let list =
                            data.data.retweeters_timeline.timeline.instructions.find(
                                (i) => i.type === "TimelineAddEntries"
                            );
                        if (!list)
                            return resolve({ list: [], cursor: undefined });
        ...
        getQuotes: (id, cursor) => {
            return new Promise((resolve, reject) => {
                ...
                fetch(
                    `/i/api/graphql/flaR-PUMshxFWZWPNpq4zA/SearchTimeline?variables=${encodeURIComponent(
                        JSON.stringify(variables)
                    )}&features=${encodeURIComponent(
                        JSON.stringify({
                            responsive_web_graphql_exclude_directive_enabled: true,
                    ...
                    .then((i) => i.json())
                    .then((data) => {
                        debugLog("tweet.getQuotes", "start", {
                            id,
                            cursor,
                            data,
                        });
                        ...
                        if (!newCursor) {
                            let replacerEntry =
                                data.data.search_by_raw_query.search_timeline.timeline.instructions.find(
                                    (i) =>
                                        i.entry_id_to_replace &&
                                        i.entry_id_to_replace.startsWith(
                                            "cursor-bottom-"
                                        )
                                );
                            if (replacerEntry) {
        ...
        lookup: (ids) => {
            ...
            return new Promise((resolve, reject) => {
                fetch(
                    `https://api.${
                        location.hostname
                    }/1.1/statuses/lookup.json?id=${ids.join(
                        ","
                    )}&include_entities=true&include_ext_alt_text=true&include_card_uri=true&tweet_mode=extended&include_reply_count=true&ext=views%2CmediaStats%2CverifiedType%2CisBlueVerified`,
                    {
                        headers: {
                            authorization: OLDTWITTER_CONFIG.public_token,
                            "x-csrf-token": OLDTWITTER_CONFIG.csrf,
        ...
        translate: (id) => {
            return new Promise((resolve, reject) => {
                chrome.storage.local.get([`translations`], async (d) => {
                    ...
                    if (
                        d.translations[id] &&
                        Date.now() - d.translations[id].date < 60000 * 60 * 4
                    ) {
                    ...
                    // Translate by Google
                    let res =
                        translateLimit > Date.now()
                            ? { ok: false }
                            : await fetch(
                                  `/i/api/1.1/strato/column/None/tweetId=${id},destinationLanguage=None,translationSource=Some(Google),feature=None,timeout=None,onlyCached=None/translation/service/translateTweet`,
                                  {
                                      headers: {
                                          authorization:
                                              OLDTWITTER_CONFIG.public_token,
                    ...
                    if (!res.ok) {
                        ...
                        if (res.headers) {
                            ...
                            let limitRemaining = res.headers.get(
                                "x-rate-limit-remaining"
                            );
                            if (
                                resetTime &&
                                limitRemaining &&
                                parseInt(limitRemaining) === 0
                            ) {
                        ...
                        res = await fetch(
                            `https://api.${location.hostname}/1.1/translations/show.json?id=${id}&dest=${l}&use_display_text=true&cards_platform=Web-13&include_entities=1&include_user_entities=1&include_cards=1&send_error_codes=1&tweet_mode=extended&include_ext_alt_text=true&include_reply_count=true`,
                            {
                                headers: {
                                    authorization:
                                        OLDTWITTER_CONFIG.public_token,
                    ...
                    let out = {
                        translated_lang: data.localizedSourceLanguage
                            ? data.localizedSourceLanguage
                            : data.translated_lang,
                        lang_code: data.sourceLanguage
                            ? data.sourceLanguage
                            : data.translated_lang,
        ...
        unmoderate: (id) => {
            return new Promise((resolve, reject) => {
                fetch(`/i/api/graphql/pVSyu6PA57TLvIE4nN2tsA/UnmoderateTweet`, {
                    ...
                    credentials: "include",
                    body: JSON.stringify({
                        variables: { tweetId: "1683331680751308802" },
        ...
        getModeratedReplies: (id, cursor) => {
            return new Promise((resolve, reject) => {
                ...
                if (cursor) variables.cursor = cursor;
                fetch(
                    `/i/api/graphql/SiKS1_3937rb72ytFnDHmA/ModeratedTimeline?variables=${encodeURIComponent(
                        JSON.stringify(variables)
                    )}&features=${encodeURIComponent(
                        JSON.stringify({
                            rweb_lists_timeline_redesign_enabled: false,
                    ...
                    .then((data) => {
                        ...
                        let entries =
                            data.data.tweet.result.timeline_response.timeline.instructions.find(
                                (i) => i.entries
                            );
                        if (!entries)
                            return resolve({
                        ...
                        if (!cursor) {
                            let entries =
                                data.data.tweet.result.timeline_response.timeline.instructions.find(
                                    (i) =>
                                        i.replaceEntry &&
                                        i.replaceEntry.entryIdToReplace.includes(
                                            "cursor-bottom"
                                        )
                                );
                            if (entries) {
                                cursor =
                                    entries.replaceEntry.entry.content.operation
                                        .cursor.value;
                            }
                        } else {
    ...
    search: {
            ...
            return new Promise((resolve, reject) => {
                fetch(
                    `/i/api/2/search/adaptive.json?${
                        ...
                        obj.tweet_search_mode
                            ? `tweet_search_mode=${obj.tweet_search_mode}&`
                            : ""
                    }include_profile_interstitial_type=1&include_blocking=1&include_blocked_by=1&include_followed_by=1&include_want_retweets=1&include_mute_edge=1&include_can_dm=1&include_can_media_tag=1&include_ext_has_nft_avatar=1&skip_status=1&cards_platform=Web-12&include_cards=1&include_ext_alt_text=true&include_quote_count=true&include_reply_count=1&tweet_mode=extended&include_ext_collab_control=true&include_entities=true&include_user_entities=true&include_ext_media_color=true&include_ext_media_availability=true&include_ext_sensitive_media_warning=true&include_ext_trusted_friends_metadata=true&send_error_codes=true&simple_quoted_tweet=true&q=${
                        obj.q
                    }${
                        obj.social_filter
                            ? `&social_filter=${obj.social_filter}`
                            : ""
                    }${
                        obj.result_filter
                            ? `&result_filter=${obj.result_filter}`
                            : ""
                    }&count=50&query_source=typed_query&pc=1&spelling_corrections=1&include_ext_edit_control=false&ext=views%2CmediaStats%2CverifiedType%2CisBlueVerified%2ChighlightedLabel%2ChasNftAvatar%2CvoiceInfo%2Cenrichments%2CsuperFollowMetadata%2CunmentionInfo%2Ccollab_control`,
                    {
                        headers: {
                            authorization: OLDTWITTER_CONFIG.public_token,
                            "x-csrf-token": OLDTWITTER_CONFIG.csrf,
                            ...
                            list: list
                                .map((e) => {
                                    if (
                                        ...
                                        let user =
                                            users[
                                                e.content.item.content.user.id
                                            ];
                                        user.id_str =
                                            e.content.item.content.user.id;
                                        user.type = "user";
        ...
        adaptiveV2: (obj, cursor) => {
            return new Promise((resolve, reject) => {
                if (cursor) obj.cursor = cursor;
                fetch(
                    `/i/api/graphql/TQmyZ_haUqANuyBcFBLkUw/SearchTimeline?variables=${encodeURIComponent(
                        JSON.stringify(obj)
                    )}&features=${encodeURIComponent(
                        JSON.stringify({
                            rweb_tipjar_consumption_enabled: true,
                    ...
                    .then((data) => {
                        ...
                        for (let entry of entries) {
                            if (
                                ...
                                user.type = "user";
                                user.socialContext =
                                    entry.content.itemContent.socialContext;
                                res.push(user);
                            }
                        }
                        let cursorBottom = entries.find(
                            (e) =>
                                e.entryId.startsWith("sq-cursor-bottom-") ||
                                e.entryId.startsWith("cursor-bottom-")
                        );
                        if (cursorBottom) {
                            ...
                            cursorBottom = instructions.find(
                                (e) =>
                        ...
                        debugLog("search.adaptiveV2", "end", {
                            obj,
                            cursor,
                            res,
                            cursorBottom,
            ...
            return new Promise((resolve, reject) => {
                chrome.storage.local.get(["savedSearches"], (d) => {
                    if (
                        cache &&
                        d.savedSearches &&
                        Date.now() - d.savedSearches.date < 60000
                    ) {
        ...
        trustedFriendsTypeahead: (circle_id, query) => {
            return new Promise((resolve, reject) => {
                ...
                fetch(
                    `/i/api/graphql/4lk-D0Y8kfimSyPJjEocsA/TrustedFriendsTypeahead?variables=${encodeURIComponent(
                        JSON.stringify(variables)
                    )}`,
                    {
                        headers: {
                    ...
                    .then((data) => {
                        ...
                        resolve(
                            data.data.trusted_friends_list_by_rest_id.recommended_members_typeahead_results.map(
                                (i) => i.result
                            )
                        );
                    })
    ...
    inbox: {
            ...
            return new Promise((resolve, reject) => {
                chrome.storage.local.get(["inboxData"], (d) => {
                    if (
                        !max_id &&
                        d.inboxData &&
                        Date.now() - d.inboxData.date < 18000
                    ) {
                    ...
                    fetch(
                        `https://api.${
                            location.hostname
                        }/1.1/dm/user_inbox.json?max_conv_count=20&include_groups=true${
                            max_id ? `&max_id=${max_id}` : ""
                        }&cards_platform=Web-13&include_entities=1&include_user_entities=1&include_cards=1&send_error_codes=1&tweet_mode=extended&include_ext_alt_text=true&include_reply_count=true`,
                        {
                            headers: {
                                authorization: OLDTWITTER_CONFIG.public_token,
                                "x-csrf-token": OLDTWITTER_CONFIG.csrf,
        ...
        markRead: (eventId) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `https://api.${location.hostname}/1.1/dm/conversation/mark_read.json`,
                    {
                        headers: {
                            authorization: OLDTWITTER_CONFIG.public_token,
                            "x-csrf-token": OLDTWITTER_CONFIG.csrf,
            ...
            return new Promise((resolve, reject) => {
                chrome.storage.local.get(["userUpdates"], (d) => {
                    ...
                    if (
                        ...
                        Date.now() - d.userUpdates[cursor].date < 4000
                    ) {
    ...
    bookmarks: {
        get: (cursor) => {
            return new Promise((resolve, reject) => {
                ...
                if (cursor) obj.cursor = cursor;
                fetch(
                    `/i/api/graphql/3OjEFzT2VjX-X7w4KYBJRg/Bookmarks?variables=${encodeURIComponent(
                        JSON.stringify(obj)
                    )}&features=${encodeURIComponent(
                        JSON.stringify({
                    ...
                    .then((data) => {
                        ...
                        let list =
                            data.data.bookmark_timeline_v2.timeline.instructions.find(
                                (i) => i.type === "TimelineAddEntries"
                            );
                        if (!list)
                            return resolve({ list: [], cursor: undefined });
        ...
        deleteAll: () => {
            return new Promise((resolve, reject) => {
                fetch(
                    `/i/api/graphql/skiACZKC1GDYli-M8RzEPQ/BookmarksAllDelete`,
                    {
                        headers: {
                            authorization: OLDTWITTER_CONFIG.public_token,
                            "x-csrf-token": OLDTWITTER_CONFIG.csrf,
    ...
    list: {
        getTweets: (id, cursor) => {
            return new Promise((resolve, reject) => {
                ...
                if (cursor) obj.cursor = cursor;
                fetch(
                    `/i/api/graphql/2Vjeyo_L0nizAUhHe3fKyA/ListLatestTweetsTimeline?variables=${encodeURIComponent(
                        JSON.stringify(obj)
                    )}&features=${encodeURIComponent(
                        JSON.stringify({
                            rweb_lists_timeline_redesign_enabled: false,
                    ...
                    .then((data) => {
                        ...
                        for (let e of list) {
                            if (e.entryId.startsWith("tweet-")) {
                                ...
                                e.entryId.startsWith("list-conversation-")
                            ) {
                                ...
                                for (let i = 0; i < lt.length; i++) {
                                    ...
                                    if (t.entryId.includes("-tweet-")) {
                                        let res =
                                            t.item.itemContent.tweet_results
                                                .result;
                                        let tweet = parseTweet(res);
                                        if (!tweet) continue;
                                        if (i !== lt.length - 1) {
        ...
        getMembers: (id, cursor) => {
            return new Promise((resolve, reject) => {
                ...
                if (cursor) obj.cursor = cursor;
                fetch(
                    `/i/api/graphql/sXFXEmtFr3nLyG1dmS81jw/ListMembers?variables=${encodeURIComponent(
                        JSON.stringify(obj)
                    )}&features=${encodeURIComponent(
                        JSON.stringify({
                            responsive_web_graphql_timeline_navigation_enabled: false,
                    ...
                    .then((i) => i.json())
                    .then((data) => {
                        debugLog("list.getMembers", "start", {
                            id,
                            cursor,
                            data,
                        });
                        ...
                        let list =
                            data.data.list.members_timeline.timeline.instructions.find(
                                (i) => i.type === "TimelineAddEntries"
                            );
                        if (!list)
                            return resolve({ list: [], cursor: undefined });
                        ...
                        let out = {
                            list: list
                                ...
                                .map((u) => {
                                    let res =
                                        u.content.itemContent.user_results
                                            .result;
        ...
        getFollowers: (id, cursor) => {
            return new Promise((resolve, reject) => {
                ...
                if (cursor) obj.cursor = cursor;
                fetch(
                    `/i/api/graphql/LxXoouvfd5E8PXsdrQ0iMg/ListSubscribers?variables=${encodeURIComponent(
                        JSON.stringify(obj)
                    )}&features=${encodeURIComponent(
                        JSON.stringify({
                            responsive_web_graphql_timeline_navigation_enabled: false,
                    ...
                    .then((i) => i.json())
                    .then((data) => {
                        debugLog("list.getFollowers", "start", {
                            id,
                            cursor,
                            data,
                        });
                        ...
                        let list =
                            data.data.list.subscribers_timeline.timeline.instructions.find(
                                (i) => i.type === "TimelineAddEntries"
                            );
                        if (!list)
                            return resolve({ list: [], cursor: undefined });
        ...
        get: (id) => {
            return new Promise((resolve, reject) => {
                chrome.storage.local.get(["listData"], (d) => {
                    ...
                    if (
                        d.listData[id] &&
                        Date.now() - d.listData[id].date < 60000
                    ) {
                        debugLog("list.get", "cache", id, d.listData[id].data);
                        return resolve(d.listData[id].data);
                    }
                    fetch(
                        `/i/api/graphql/vxx-Y8zadpAP64HHiw4hMQ/ListByRestId?variables=${encodeURIComponent(
                            JSON.stringify({
                                listId: id,
                                withSuperFollowsUserFields: true,
                            })
            ...
            return new Promise((resolve, reject) => {
                fetch(`/i/api/graphql/nymTz5ek0FQPC3kh63Tp1w/ListSubscribe`, {
                    ...
                    body: JSON.stringify({
                        ...
                        queryId: "nymTz5ek0FQPC3kh63Tp1w",
                    }),
                    credentials: "include",
                })
                    .then((i) => i.json())
            ...
            return new Promise((resolve, reject) => {
                fetch(`/i/api/graphql/Wi5-aG4bvTmdjyRyRGkyhA/ListUnsubscribe`, {
                    ...
                    body: JSON.stringify({
                        ...
                        queryId: "Wi5-aG4bvTmdjyRyRGkyhA",
                    }),
                    credentials: "include",
                })
                    .then((i) => i.json())
            ...
            return new Promise((resolve, reject) => {
                fetch(`/i/api/graphql/RKtQuzpcy2gym71UorWg6g/ListAddMember`, {
                    ...
                    body: JSON.stringify({
                        ...
                        queryId: "RKtQuzpcy2gym71UorWg6g",
                    }),
                    credentials: "include",
                })
                    .then((i) => i.json())
        ...
        removeMember: (listId, userId) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `/i/api/graphql/mDlp1UvnnALC_EzybKAMtA/ListRemoveMember`,
                    {
                        headers: {
                            authorization: OLDTWITTER_CONFIG.public_token,
                            "x-csrf-token": OLDTWITTER_CONFIG.csrf,
                        ...
                        body: JSON.stringify({
                            ...
                            queryId: "mDlp1UvnnALC_EzybKAMtA",
                        }),
                        credentials: "include",
                    }
                )
        ...
        getMyLists: () => {
            return new Promise((resolve, reject) => {
                chrome.storage.local.get(["myLists"], (d) => {
                    ...
                    fetch(
                        `/i/api/graphql/cl2dF-zeGiLvZDsMGZhL4g/ListsManagementPageTimeline?variables=${encodeURIComponent(
                            JSON.stringify({
                                count: 100,
                                withSuperFollowsUserFields: true,
                                withDownvotePerspective: false,
            ...
            return new Promise((resolve, reject) => {
                fetch(`/i/api/graphql/x5aSMDodNU02VT1VRyW48A/CreateList`, {
                    ...
                    body: JSON.stringify({
                        ...
                        queryId: "x5aSMDodNU02VT1VRyW48A",
                    }),
                    credentials: "include",
                })
                    .then((i) => i.json())
        ...
        getOwnerships: (myId, userId) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `/i/api/graphql/6E69fsenLDPDcprqtogzdw/ListOwnerships?variables=${encodeURIComponent(
                        JSON.stringify({
                            userId: myId,
    ...
    circle: {
        getCircles: () => {
            return new Promise((resolve, reject) => {
                fetch(
                    `/i/api/graphql/QjN8ZdavFDqxUjNn3r9cig/AuthenticatedUserTFLists?variables=%7B%7D`,
                    {
                        headers: {
                            authorization: OLDTWITTER_CONFIG.public_token,
                            "x-csrf-token": OLDTWITTER_CONFIG.csrf,
                    ...
                    .then((data) => {
                        ...
                        resolve(
                            data.data.authenticated_user_trusted_friends_lists
                        );
                    })
                    .catch((e) => {
                        reject(e);
        ...
        getMembers: (id, cursor = null) => {
            return new Promise((resolve, reject) => {
                ...
                fetch(
                    `/i/api/graphql/i3_opgZeSaeWbfyFQjZ5Sw/TrustedFriendsMembersQuery?variables=${encodeURIComponent(
                        JSON.stringify(variables)
                    )}&features=${encodeURIComponent(
                        JSON.stringify(features)
                    )}`,
                    ...
                    .then((data) => {
                        ...
                        resolve(
                            data.data.trusted_friends_list_by_rest_id.members_slice.items_results
                                .filter(
                                    (u) =>
                                        u.result &&
                                        u.result.is_trusted_friends_list_member
                                )
                                .map((u) => u.result)
                        );
                    })
        ...
        removeUser: (circle_id, circle_rest_id, item_id, user_id) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `/i/api/graphql/fl9NbcQB1UE5uiYvEHfHGA/TrustedFriendsAddRemoveButtonRemoveMutation`,
                    {
                        method: "POST",
                        headers: {
                            authorization: OLDTWITTER_CONFIG.public_token,
        ...
        addUser: (circle_id, circle_rest_id, user_id) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `/i/api/graphql/QFcDZhljP_e9bzeT8saZ3A/TrustedFriendsAddRemoveButtonAddMutation`,
                    {
                        method: "POST",
                        headers: {
                            authorization: OLDTWITTER_CONFIG.public_token,
    ...
    topic: {
        landingPage: (id, cursor) => {
            return new Promise((resolve, reject) => {
                ...
                if (cursor) variables.cursor = cursor;
                fetch(
                    `/i/api/graphql/4exqISyA1-LejxLHY4RqJA/TopicLandingPage?variables=${encodeURIComponent(
                        JSON.stringify(variables)
                    )}&features=${encodeURIComponent(
                        JSON.stringify({
                            responsive_web_graphql_timeline_navigation_enabled: false,
        ...
        notInterested: (id) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `/i/api/graphql/cPCFdDAaqRjlMRYInZzoDA/TopicNotInterested`,
                    {
                        method: "POST",
                        headers: {
                            authorization: OLDTWITTER_CONFIG.public_token,
        ...
        undoNotInterested: (id) => {
            return new Promise((resolve, reject) => {
                fetch(
                    `/i/api/graphql/4tVnt6FoSxaX8L-mDDJo4Q/TopicUndoNotInterested`,
                    {
                        method: "POST",
                        headers: {
                            authorization: OLDTWITTER_CONFIG.public_token,
    ...
    uploadMedia: (data) => {
        return new Promise(async (resolve, reject) => {
            ...
            async function tryInitializing() {
                ...
                if (initAttempts++ > 7)
                    return reject("Failed to initialize media upload");
                try {
                    initUpload = await fetch(
                        `https://upload.${location.hostname}/1.1/media/upload.json`,
                        {
                            headers: {
                                authorization: OLDTWITTER_CONFIG.public_token,
                                "x-csrf-token": OLDTWITTER_CONFIG.csrf,
            ...
            let segmentSize =
                ...
                !vars.disableDataSaver
                    ? 1084576 / 2
                    : 1084576; // smaller chunks for cellular data
            let segmentCount = Math.ceil(data.media.byteLength / segmentSize);
            for (let i = 0; i < segmentCount; i++) {
                let segmentData = data.media.slice(
                    i * segmentSize,
                    (i + 1) * segmentSize
                );
            ...
            for (let i in segments) {
                ...
                async function tryUploadingChunk() {
                    ...
                    fd.append(
                        "media",
                        new Blob([segment], { type: data.media_type })
                    );
                    try {
                        await fetch(
                            `https://upload.${location.hostname}/1.1/media/upload.json?command=APPEND&media_id=${mediaId}&segment_index=${i}`,
                            {
                                headers: {
                                    authorization:
                                        OLDTWITTER_CONFIG.public_token,
            ...
            async function tryFinalizing() {
                if (data.loadCallback)
                    data.loadCallback({
                        text:
                            LOC.finalization.message +
                            ".".repeat(finalAttempts),
                        progress: 100,
                    });
                if (finalAttempts++ > 7)
                    return reject("Failed to finalize media upload");
                try {
            ...
            async function checkStatus() {
                if (statusTries++ > 60 * 20)
                    return clearInterval(statusInterval);
                await fetch(
                    `https://upload.${
                        location.hostname
                    }/1.1/media/upload.json?${new URLSearchParams({
                        command: "STATUS",
                        media_id: mediaId,
                    }).toString()}`,
                    {
                    ...
                    .then((i) => {
                        ...
                        if (i.processing_info.state === "failed") {
                            if (i.processing_info.error.message) {
                                ...
                                reject(
                                    `Twitter API rejected your media with code ${i.processing_info.error.code} (${i.processing_info.error.name})`
                                );
                            }
                        }
                        if (i.processing_info.state === "in_progress") {
                            if (
                                !i.processing_info.check_after_secs &&
                                i.processing_info.error
                            ) {
                                if (i.processing_info.error.message) {
                                    return reject(
                                        i.processing_info.error.message
                                    );
                                } else {
                                    return reject(
                                        `Twitter API rejected your media with code ${i.processing_info.error.code} (${i.processing_info.error.name})`
                            ...
                            setTimeout(
                                checkStatus,
                                i.processing_info.check_after_secs * 1000
                            );
                            if (data.loadCallback) {
                                data.loadCallback({
                                    text: LOC.processing.message,
                                    progress:
                                        i.processing_info.progress_percent,
                                });
                            }
                        }
                    });
